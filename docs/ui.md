...
- Value label at the intersection of cursor and signal curve; which cursor(s) show it is controlled by Preferences → Cursors → "Display values at cursors" (#174): Active (default) shows only whichever cursor is currently closer to the mouse pointer, Both shows both cursors' labels at once, Neither hides plot-canvas value labels entirely.
...
- **Cursors** — cursor mode, "persistent" toggle, 4 color swatches (Cursor 1 / Cursor 2 / Cursor Left / Cursor Right chevrons), "Show ∆-Time" checkbox + its own color swatch, "Display values at cursors" combo box (Neither/Active/Both, #174 — which cursor(s)' plot-canvas value labels are shown; defaults to Active, today's hover-based single-cursor behavior; doesn't affect ∆-Time or the Active Signals Table's own cursor columns), arrow-key step size (unit combo box + spinbox), reset-to-defaults button
