...
- **File**
  ...
  - Exit (Ctrl+Q)
- **Edit**
  - New Tab (Ctrl+T; Ctrl+N on the MDA shortcut preset) / New Stripe (Ctrl+Shift+N) (#115 — moved here from File)
...
- **All Stripes** (A) – checkable; whether "Zoom to Fit"/"Zoom Y to View" apply to every stripe or only the active one (Swimlanes and box-zoom are always scoped to the stripe they're used in, regardless of this toggle)
...
- Right-click context menu: Copy Name(s) (#163), Remove Signal(s), Toggle Visibility (#133), Y Autozoom (#142), Search… (#110 — opens the Search dialog pre-filled for the selected signal(s), see "Search Dialog" below), Enable/Disable Step Mode, Shorten Signal Names (toggle), Display Name Rule…, Merge Y-Axis / Sync Y-Axis / Remove from merged-synced axis (2+ signals), Move to Stripe / Move to new Stripe.
...
- On a match, Cursor 1 (or Cursor L, if delta-time cursor labeling is on) jumps to that timestamp — auto-enabling from hidden if needed, leaving Cursor 2/R untouched — and the plot recenters on it at the current zoom width, across every stripe in the tab (they share one X-axis). The dialog stays open either way; when nothing matches, an inline "No match found" message appears instead.
