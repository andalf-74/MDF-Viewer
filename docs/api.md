...
- **File**: Open… (Ctrl+O) → Save Config (Ctrl+S) → Save Config As… → Close Measurement (submenu, rebuilt on `aboutToShow`, #103) → separator → [recent files, dynamically inserted here on `aboutToShow`] → separator → Preferences… → separator → Exit (Ctrl+Q)
